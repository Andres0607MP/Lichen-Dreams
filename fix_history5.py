...
if old in content:
    ...
    # Try the actual content from file
    idx = content.find('                 const SizedBox(width: 8),\n                  Wrap(')
    if idx >= 0:
        actual = content[idx:idx+len(old)]
        print("Comparing...")
        for i, (a, b) in enumerate(zip(actual, old)):
            if a != b:
                print(f"Mismatch at {i}: actual={repr(a)} vs old={repr(b)}")
                print(f"Context: {repr(actual[max(0,i-20):i+20])}")
                break