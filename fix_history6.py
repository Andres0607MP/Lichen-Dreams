...
# Get the exact old block from the file
start = content.find('                 const SizedBox(width: 8),\n                  Wrap(')
end = content.find('Future<void> _showEnvironmentalChartSheet', start)
old = content[start:end]

print(f"Old block length: {len(old)}")
...
print("SUCCESS: Replacement done using exact match from file")