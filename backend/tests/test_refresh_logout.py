...
def test_refresh_and_logout_flow():
    ...
    reg = client.post("/auth/register", json={"email": email, "password": password, "name": "Test User"})
    assert reg.status_code in (200, 201)

    # login (uses form data per backend contract)
    ...
    out = client.post("/auth/logout_refresh", json={"refresh_token": refresh_token})
